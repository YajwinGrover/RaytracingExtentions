...
class lambertian : public material {
    public:
        ...
        virtual bool scatter(const ray& r, const hit_record& record, color& attenuation, ray& scattered) const override {
            auto scatter_direction = record.normal + random_unit_vector();
            if(scatter_direction.near_zero()){
                scatter_direction = record.normal;
            }
            scattered = ray(record.p, scatter_direction, r.get_time());
...
class metal : public material {
    public:
        ...
        virtual bool scatter(const ray& r, const hit_record& rec, color& attenuation, ray& scattered) const override {
            vec3 reflected = reflect(unit_vector(r.get_direction()), rec.normal);
            scattered = ray(rec.p, reflected + fuzz*random_in_unit_sphere(), r.get_time());
            attenuation = albedo;
...
class dielectric : public material{
    public:
        ...
        virtual bool scatter(const ray& r, const hit_record& rec, color& attenuation, ray& scattered) const override {
            ...
            scattered = ray(rec.p, direction, r.get_time());
            return true;
        }
...
class diffuse_light : public material {
	public:
		...
		diffuse_light(shared_ptr<texture> emit_material) : emit(emit_material) {}
		diffuse_light(color emit_color) : emit(make_shared<solid_color>(emit_color)) {}
...
class isotropic : public material {
    public:
        ...
        virtual bool scatter(const ray& ray_in, const hit_record& rec, color& attenuation, ray& scattered) const override {
            scattered = ray(rec.p, random_in_unit_sphere(), ray_in.get_time());
